 ...
 *
 * <ul>
 *   <li>{@code SkyRenderer#renderSkyDisc(RenderPass, Vector3fc)} … 空ドームの色</li>
...
public class DarkerSky implements ClientModInitializer {
	...
	/**
	 * 空ドームの基本色を現在の {@link #getDarkness()} 倍した新しいベクトルを返します。
	 *
	 * <p>引数の {@code color} は {@code SkyRenderState} が保持しているインスタンスなので、
	 * 破壊的に書き換えず必ず新しい {@link Vector3f} を返します。</p>
	 */
	public static Vector3f darken(Vector3fc color) {
		float d = darkness;
		return new Vector3f(color.x() * d, color.y() * d, color.z() * d);
	}
}
