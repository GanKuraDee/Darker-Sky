...
A client-side Fabric mod that darkens the sky. Targets Minecraft **26.3**.
...
|---|---|---|
| Sky dome (upper sky) | the `skyColor` argument of `SkyRenderer#renderSkyDisc(RenderPass, Vector3fc skyColor)` | `SkyRendererMixin` |
...
- In 26.3 the sky disc is drawn through a `RenderPass` and its color comes from `SkyRenderState#skyColor` as a `Vector3fc`, so `SkyRendererMixin` differs from the 26.1.x / 26.2 builds (which modified an ARGB `int`). `FogRendererMixin`, `ChatScreenMixin`, and `ClientPacketListenerMixin` are unchanged from 26.2.
...
Built against the Mod Menu **20.0.1** API (`compileOnly`); use the Mod Menu build that matches Minecraft 26.3 at runtime.
